...
class MasqueradeWidget extends Component {
  ...
  componentDidMount() {
    getMasqueradeOptions(this.courseId)
      .then((data) => {
        if (data.success) {
          this.onSuccess(data);
        } else {
      ...
      .catch(() => {}); // Removed unused 'response' parameter
  }
  ...
  parseAvailableOptions(postData) {
    ...
    if (active.userName) {
      ...
      // Removed unused 'masquerade' state update
  ...
  render() {
    const { autoFocus, shouldShowUserNameInput, masqueradeUsername } = this.state;
